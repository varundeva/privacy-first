...
interface BmpToJpgConverterProps {
  file: File;
  onReset: () => void;
}
 ...
 * 
 * Converts BMP bitmap images to compressed JPEG format.
 * Dramatically reduces file size with high quality output.
 * Perfect for sharing and web use.
 */
export function BmpToJpgConverter({ file, onReset }: BmpToJpgConverterProps) {
    ...
    const processFile = async () => {
      try {
        ...
        if (result.success && result.data) {
          ...
          setState({
            status: 'error',
            message: result.error || 'Failed to convert BMP to JPG',
          });
        }
      } catch (error) {
        if (!cancelled) {
