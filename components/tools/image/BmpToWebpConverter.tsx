...
interface BmpToWebpConverterProps {
  file: File;
  onReset: () => void;
}
 ...
 * 
 * Converts BMP bitmap images to modern WebP format.
 * Achieves maximum compression with excellent quality.
 * Best choice for web optimization.
 */
export function BmpToWebpConverter({ file, onReset }: BmpToWebpConverterProps) {
    ...
    const processFile = async () => {
      try {
        ...
        if (result.success && result.data) {
          ...
          setState({
            status: 'error',
            message: result.error || 'Failed to convert BMP to WebP',
          });
        }
      } catch (error) {
        if (!cancelled) {
