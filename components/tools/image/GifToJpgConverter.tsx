...
interface GifToJpgConverterProps {
  file: File;
  onReset: () => void;
}
 ...
 * 
 * Converts GIF images to JPEG format.
 * Extracts the first frame from animated GIFs.
 * Perfect for creating thumbnails with maximum compatibility.
 */
export function GifToJpgConverter({ file, onReset }: GifToJpgConverterProps) {
    ...
    const processFile = async () => {
      try {
        ...
        if (result.success && result.data) {
          ...
          setState({
            status: 'error',
            message: result.error || 'Failed to convert GIF to JPG',
          });
        }
      } catch (error) {
        if (!cancelled) {
