...
interface GifToWebpConverterProps {
  file: File;
  onReset: () => void;
}
 ...
 * 
 * Converts GIF images to modern WebP format.
 * Extracts the first frame from animated GIFs.
 * Great for web optimization with smaller file sizes.
 */
export function GifToWebpConverter({ file, onReset }: GifToWebpConverterProps) {
    ...
    const processFile = async () => {
      try {
        ...
        if (result.success && result.data) {
          ...
          setState({
            status: 'error',
            message: result.error || 'Failed to convert GIF to WebP',
          });
        }
      } catch (error) {
        if (!cancelled) {
