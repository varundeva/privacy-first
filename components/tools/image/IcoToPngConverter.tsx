...
interface IcoToPngConverterProps {
  file: File;
  onReset: () => void;
}
 ...
 * 
 * Converts Windows icon files to PNG format.
 * Extracts icon images for general use.
 * Perfect for using icons in web projects and designs.
 */
export function IcoToPngConverter({ file, onReset }: IcoToPngConverterProps) {
  ...
  useEffect(() => {
    ...
    const processFile = async () => {
      try {
        setState({
          status: 'processing',
          progress: { percent: 0, stage: 'loading', message: 'Loading ICO file...' },
        });
        ...
        if (result.success && result.data) {
          ...
          setState({
            status: 'error',
            message: result.error || 'Failed to convert ICO to PNG',
          });
        }
      } catch (error) {
        if (!cancelled) {
