...
interface SvgToJpgConverterProps {
  file: File;
  onReset: () => void;
}
 ...
 * 
 * Rasterizes vector SVG graphics to JPEG format.
 * Creates compact bitmap images from vector files.
 * Ideal for sharing and universal compatibility.
 */
export function SvgToJpgConverter({ file, onReset }: SvgToJpgConverterProps) {
    ...
    const processFile = async () => {
      try {
        ...
        if (result.success && result.data) {
          ...
          setState({
            status: 'error',
            message: result.error || 'Failed to convert SVG to JPG',
          });
        }
      } catch (error) {
        if (!cancelled) {
