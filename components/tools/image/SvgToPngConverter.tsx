 ...
 * 
 * Rasterizes vector SVG graphics to PNG format.
 * Converts scalable vectors to fixed-resolution images.
 * Perfect for when you need a bitmap version of vector graphics.
 */
export function SvgToPngConverter({ file, onReset }: SvgToPngConverterProps) {
    ...
    const processFile = async () => {
      try {
        ...
        if (result.success && result.data) {
          ...
          setState({
            status: 'error',
            message: result.error || 'Failed to convert SVG to PNG',
          });
        }
      } catch (error) {
        if (!cancelled) {
