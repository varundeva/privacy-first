...
interface WebpToJpgConverterProps {
  file: File;
  onReset: () => void;
}
 ...
 * 
 * Converts WebP images to JPEG format for maximum compatibility.
 * Ideal for:
 * - Sharing images on platforms that don't support WebP
 * - Email attachments
 * - Printing services
 * - Legacy software compatibility
 */
export function WebpToJpgConverter({ file, onReset }: WebpToJpgConverterProps) {
    ...
    const processFile = async () => {
      try {
        ...
        if (result.success && result.data) {
          ...
          setState({
            status: 'error',
            message: result.error || 'Failed to convert WebP to JPG',
          });
        }
      } catch (error) {
        if (!cancelled) {
