...
interface WebpToPngConverterProps {
  file: File;
  onReset: () => void;
}
 ...
 * Use this when:
 * - You need to edit images in software that doesn't support WebP
 * - You need transparency support with maximum compatibility
 * - You want a universally supported format
 */
export function WebpToPngConverter({ file, onReset }: WebpToPngConverterProps) {
  ...
  useEffect(() => {
    ...
    const processFile = async () => {
      try {
        setState({
          status: 'processing',
          progress: { percent: 0, stage: 'loading', message: 'Loading WebP image...' },
        });
        ...
        if (result.success && result.data) {
          ...
          setState({
            status: 'error',
            message: result.error || 'Failed to convert WebP to PNG',
          });
        }
      } catch (error) {
        if (!cancelled) {
