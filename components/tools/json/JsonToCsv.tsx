    ...
    const flattenObject = (obj: any, prefix = '', res: any = {}) => {
        for (const key in obj) {
            if (Object.prototype.hasOwnProperty.call(obj, key)) {
                ...
                if (typeof val === 'object' && val !== null && !Array.isArray(val)) { // simple flatten, array handled as string or ignored?
                    flattenObject(val, newKey, res);
                } else if (Array.isArray(val)) {
                    res[newKey] = `[${val.length} items]`; // simplify array representation for CSV
                } else {
                    res[newKey] = val;
                }
            }
    ...
    const handleValidate: OnValidate = useCallback((markers) => {
        ...
        if (errors.length > 0) {
            ...
            if (input.trim().length > 0) {
                setError(null);
                setStatus('valid'); // Just syntactically valid JSON, structure valid check on convert
            }
        }
    }, [input]);
                ...
                <div className="grid lg:grid-cols-2 gap-6 h-[600px]">
                    ...
                    <Card className="flex flex-col border-2 border-border overflow-hidden h-full">
                        ...
                        <div className="flex-1">
                            <Editor
                                height="100%"
                                language="csv" // Using 'csv' language usually supported or text
                                value={output}
                                theme={editorTheme}
                                options={{
                                    readOnly: true,
