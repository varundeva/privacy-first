 ...
 * 
 * Converts PDF document pages to high-quality PNG images.
 * Perfect for presentations, document sharing, and archival.
 * All processing happens in the browser for maximum privacy.
 */
export function PdfToPngConverter({ file, onReset }: PdfToPngConverterProps) {
  const [state, setState] = useState<ConversionState>({ status: 'idle' });
  const [previewUrls, setPreviewUrls] = useState<string[]>([]);
  const [currentPage, setCurrentPage] = useState(0);
  const [scale, setScale] = useState(2); // Default 2x scale (144 DPI)
    ...
    const processFile = async () => {
      try {
        ...
        if (result.success && result.pages) {
          ...
          setState({
            status: 'error',
            message: result.error || 'Failed to convert PDF to PNG',
          });
        }
      } catch (error) {
        if (!cancelled) {
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground">
              {pages.length} page{pages.length > 1 ? 's' : ''} converted to PNG
            </p>
          </div>
          <div className="text-right text-sm text-muted-foreground">
            <p>Original: {formatFileSize(file.size)}</p>
            <p>Total PNG: {formatFileSize(totalSize)}</p>
          </div>
        </div>
      </Card>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        {pages.length > 1 && (
          ...
          >
            <Download className="h-4 w-4" />
            Download All Pages ({pages.length} PNG files)
          </Button>
        )}
